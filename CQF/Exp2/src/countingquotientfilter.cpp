...
uint32_t FILTER_BIT_SIZE = (uint32_t) ((1LL << 20) * 16);
double FALSE_POSITIVE_RATE = 1E-4;
uint32_t BITS_PER_FINGERPRINT = 16;
uint32_t FILTER_MAX_ITEMS = (uint32_t) floor(FILTER_LOAD_FACTOR * FILTER_BIT_SIZE / (BITS_PER_FINGERPRINT + 2.125));
...
CountingQuotientFilter::CountingQuotientFilter(uint32_t max_items)
{
    ...
    num_items = 0;
    num_slots = (uint32_t) floor(1.0 * FILTER_MAX_ITEMS / FILTER_LOAD_FACTOR);
    num_blocks = (uint32_t) floor(1.0 * num_slots / SLOTS_PER_BLOCK);
    ...
    fingerprint_array = new uint16_t [num_slots];
    memset(fingerprint_array, 0, sizeof(uint16_t) * num_slots);
    filter_size = num_blocks * (sizeof(uint64_t) + sizeof(uint64_t) + sizeof(uint8_t)) + num_slots * sizeof(uint16_t);
...
{
    fingerprint_array[slot_index] = (uint16_t) fingerprint_key;
}
...
{
    fingerprint_key = fingerprint_array[slot_index];
}
