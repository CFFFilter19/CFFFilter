...
#define MAX_SIZE_RUNS       3  // 4Mb, 16Mb, 64Mb.
#define MAX_TEST_RUNS       10  // 10 test runs.
#define MAX_FPR_RUNS        3  // 1E-2, 1E-4, 1E-6.
#define SLOT_BIT_SIZE       16  // 16 bits per slot.
uint32_t SLOT_INDEX_BITS = 18;  // 2^18=256K, 2^20=1M, 2^22=4M.
...
void TestBloomFilter()
{
    ...
    for(size_t h = 0; h < MAX_SIZE_RUNS; h++)  // 4Mb, 16Mb, 64Mb.
    {
        ...
        for(size_t i = 0; i < MAX_FPR_RUNS; i++)  // 1E-2, 1E-4, 1E-6.
        {
			...
			FILTER_MAX_ITEMS = (uint32_t) floor(FILTER_LOAD_FACTOR * FILTER_BIT_SIZE / (BITS_PER_FINGERPRINT + 2.125));
