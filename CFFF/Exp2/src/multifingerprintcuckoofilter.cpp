...
MultiFingerprintCuckooFilter::MultiFingerprintCuckooFilter(uint32_t max_items)
{
    ...
    slot_array = new uint16_t [num_slots];
    memset(slot_array, 0, sizeof(uint16_t) * num_slots);
    filter_size = sizeof(uint16_t) * num_slots;
...
{
    slot_array[(bucket_index * SLOTS_PER_BUCKET) + slot_index] = (uint16_t) fingerprint_key;
}
